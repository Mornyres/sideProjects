...
%The buck converter circuit steps the battery's 3.7V voltage down to 3.3VDC$\pm$10\% for R.F.I.D.'s microcontroller and SD card.
...
%The energy stored in this parasitic inductor is then released when the voltage across it, the voltage ripple seen at the output or input of the buck converter, is at its peak, causing voltage and current ripples. 
...
\par
%The crystal and internal circuitry form an oscillator used by the microcontroller to provide a consistent frequency.
%The RTC for the STM32l476 microcontroller requires a low speed external (LSE) oscillator.
...
%There are also different variations of FAT filesystems that R.F.I.D. could support. 
...
\textbf{Choice of microcontroller:}
 ...
 %is also commonly used for similar FAT-based file storage applications.
